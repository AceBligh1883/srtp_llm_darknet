# src/features/content_processor.py
"""
提供独立的内容处理与增强功能，如查询重写、图像描述等。
"""
import re
import json
import ast
...
def rewrite_query(llm_client: GeminiClient, question: str) -> List[str]:
    """使用LLM重写和扩展用户查询。"""
    prompt = prompts.QUERY_REWRITER_PROMPT.format(question=question)
    response = llm_client.generate(prompt)
    logger.debug(f"LLM为查询重写返回: '{response}'")
    try:
...
def describe_images_batch(llm_client: GeminiClient, image_processor: ImageProcessor, image_paths: List[str]) -> Dict[str, str]:
    ...
    try:
        ...
        valid_images = [img for img in (image_processor.process(p) for p in image_paths) if img]
        
        if not valid_images:
            return {path: "[图像无法处理]" for path in image_paths}

        response = llm_client.generate(prompts.BATCH_IMAGE_DESCRIPTION_PROMPT, pil_image=valid_images)
        match = re.search(r'\{.*\}', response, re.DOTALL)
        if not match:
        ...
        logger.error(f"批量描述图片时发生严重错误: {e}", exc_info=True)
        return {path: "[批量分析图像时出错]" for path in image_paths}

def analyze_query_image(llm_client: GeminiClient, image_processor: ImageProcessor, image_path: str) -> dict:
    """
    分析单个查询图片，返回其描述和关键词。
    返回一个字典: {"description": str, "keywords": List[str], "pil_image": Image}
    """
    logger.info(f"正在分析主查询图片: '{image_path}'")
    pil_image = image_processor.process(image_path)
    if not pil_image:
        raise ValueError(f"无法打开或处理查询图像: {image_path}")

    prompt = prompts.SINGLE_IMAGE_ANALYSIS_PROMPT
    response = llm_client.generate(prompt, pil_image=pil_image)
