...
class ProcessingPipeline:
    ...
    def process_directory(self, directory: str, content_type: str):
        ...
        data_to_embed_list = [item['data'] for item in valid_items_for_embedding]
        vectors = None

        if content_type == 'image':
            vectors = self.embed_generator.get_image_embeddings(data_to_embed_list)
        elif content_type == 'text':
            vectors = self.embed_generator.get_text_embeddings(data_to_embed_list)

        if not vectors or len(vectors) != len(valid_items_for_embedding):
            logger.error("向量生成失败或数量不匹配，处理中止。")
            return
        
        success_count = 0
        desc_index = f"索引 {content_type} 向量到 Elasticsearch"
        for i, item in enumerate(tqdm(valid_items_for_embedding, desc=desc_index)):
            if self.index_manager.index_document(item['doc_id'], vectors[i]):
                success_count += 1
        
        logger.info(f"目录 {os.path.basename(directory)} 处理完成。成功索引: {success_count}/{len(valid_items_for_embedding)}")